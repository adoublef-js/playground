...
export class DB {
    ...
    constructor(path: string = ":memory:", options: SqliteOptions = {}) {
        ...
        const status = setStr(this.#wasm, path, (ptr) =>
            this.#wasm.open(ptr, flags)
        );
    ...
    serialize(schema: "main" | "temp" | string = "main"): Uint8Array {
        const ptr = setStr(this.#wasm, schema, (ptr) =>
            this.#wasm.serialize(ptr)
        );
        if (ptr === Values.Null) {
    ...
    deserialize(data: Uint8Array, options?: SqliteDeserializeOptions) {
        ...
        const status = setStr(this.#wasm, schema, (schemaPtr) =>
    ...
    >(func: (...args: A) => R, options?: SqliteFunctionOptions) {
        ...
        const status = setStr(this.#wasm, name, (name) =>
            this.#wasm.create_function(name, argc, flags, funcIdx)
        );
    ...
    deleteFunction(name: string) {
        if (this.#functionNames.has(name)) {
            const status = setStr(this.#wasm, name, (pts) =>
                this.#wasm.delete_function(pts)
            );
            if (status === Status.SqliteOk) {
